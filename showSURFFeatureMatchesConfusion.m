function showSURFFeatureMatchesConfusion(bag, trainingSet, trainSets, trainFeatures,...
...
for i=1:n
    ...
    rightTestFiles = M( string(M(:,3)) == string(labels(i)), : );
    ...
    for k=1:nFiles
        ...
        for l=1:mFiles
            ...
            if ~isempty(img1Features) && ~isempty(img2Features)
                ...
                weighted_sum = sum(img2Metrics.Metric(index_pairs(:,2)));
    ...
    showSURFFeatureMatches(bag, img1, img2, mkLabel, labels(i), outFolder, mkTableRow{4+i});
